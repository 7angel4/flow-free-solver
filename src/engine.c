...
int game_can_move(const game_info_t* info,
	...
	// MODIFIED: Make sure color is valid
...
int game_next_move_color(const game_info_t* info,
	...
	// stay with the last color moved if it's valid and not completed
	...
	if ( g_options.order_most_constrained) {
		...
		// REMOVED: assert(best_color < info->num_colors);
		// let the function return -1 if no color left to pick
		return best_color;
...
//////////////////////////////////////////////////////////////////////
// Return free if the position is in bounds and unoccupied

int game_pos_is_free(const game_info_t* info,
					 const game_state_t* state,
					 pos_t pos) {

	int x, y;
	pos_get_coords(pos, &x, &y);
	return game_is_free(info, state, x, y);
}

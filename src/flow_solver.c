...
int main(int argc, char** argv) {
	...
	for (size_t i=0; i<num_inputs; ++i) {
		...
		if (game_read(input_file, &info, &state)) {
			...
			if (!g_options.display_quiet) {
				...
				printf("\nsearch %s after %'.7f seconds and %'zu nodes (%'.2f MB)\n",
	...
	if (boards > 1) {
		...
		if (!g_options.display_quiet) {
			...
			for (int i=0; i<3; ++i) {
				if (total_count[i]) {
					printf("%'d %s searches took a total of %'.7f seconds and %'zu nodes\n",
			...
			if (types > 1) {
				printf("\n");
				printf("overall, %'d searches took a total of %'.7f seconds "
				       "and %'zu nodes\n",
			...
			for (int i=0; i<3; ++i) {
				if (total_count[i]) {
					printf("%*s%3d total %c %'12.7f %'12zu\n",
					       max_width-9, "",
					       total_count[i],
			...
			if (types > 1) {
				printf("\n");
				printf("%*s%3d overall %'12.7f %'12zu\n",
				       max_width-9, "",
				       boards,
				       overall_elapsed,
				       overall_nodes);
